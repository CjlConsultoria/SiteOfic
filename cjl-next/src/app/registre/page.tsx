...
export default function RegistrePage() {
  ...
  const inputClass = "w-full px-4 py-3 rounded-lg bg-gradient-to-b from-[#eff6ff] to-white border border-white/[0.08] text-white placeholder:text-gray-600 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition"

  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-20 bg-gradient-to-b from-[#eff6ff] to-white">
        ...
        <div className="bg-white border border-gray-200 shadow-xl rounded-2xl p-8">
          ...
          {step === 0 && (
            <div className="space-y-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Nome *</label><input type="text" value={form.nome} onChange={e => update('nome', e.target.value)} placeholder="Seu nome" className={inputClass} /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Sobrenome *</label><input type="text" value={form.sobrenome} onChange={e => update('sobrenome', e.target.value)} placeholder="Seu sobrenome" className={inputClass} /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Apelido (opcional)</label><input type="text" value={form.apelido} onChange={e => update('apelido', e.target.value)} placeholder="Como quer ser chamado" className={inputClass} /></div>
            </div>
          )}

          {/* Step 1: CPF */}
          {step === 1 && (
            <div><label className="block text-sm font-semibold text-gray-700 mb-2">CPF *</label><input type="text" maxLength={11} value={form.cpf} onChange={e => update('cpf', e.target.value.replace(/\D/g, ''))} placeholder="Somente números" className={inputClass} /></div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              {form.ehPJ && (
                <>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">CNPJ</label><input type="text" maxLength={14} value={form.cnpj} onChange={e => update('cnpj', e.target.value.replace(/\D/g, ''))} placeholder="Somente números" className={inputClass} /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Nome da Empresa</label><input type="text" value={form.nomeEmpresa} onChange={e => update('nomeEmpresa', e.target.value)} placeholder="Razão social" className={inputClass} /></div>
                </>
              )}
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">CEP *</label><input type="text" maxLength={8} value={form.cep} onChange={e => { update('cep', e.target.value.replace(/\D/g, '')); }} onBlur={buscarCep} placeholder="Somente números" className={inputClass} /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Logradouro *</label><input type="text" value={form.logradouro} onChange={e => update('logradouro', e.target.value)} className={inputClass} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-semibold text-gray-700 mb-2">Número *</label><input type="text" value={form.numero} onChange={e => update('numero', e.target.value)} className={inputClass} /></div>
                <div><label className="block text-sm font-semibold text-gray-700 mb-2">Complemento</label><input type="text" value={form.complemento} onChange={e => update('complemento', e.target.value)} className={inputClass} /></div>
              </div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Bairro</label><input type="text" value={form.bairro} onChange={e => update('bairro', e.target.value)} className={inputClass} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-semibold text-gray-700 mb-2">Cidade *</label><input type="text" value={form.cidade} onChange={e => update('cidade', e.target.value)} className={inputClass} /></div>
                <div><label className="block text-sm font-semibold text-gray-700 mb-2">Estado *</label><input type="text" maxLength={2} value={form.estado} onChange={e => update('estado', e.target.value.toUpperCase())} className={inputClass} /></div>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="space-y-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">E-mail *</label><input type="email" value={form.email} onChange={e => update('email', e.target.value)} placeholder="seu@email.com" className={inputClass} /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Senha *</label><input type="password" value={form.senha} onChange={e => update('senha', e.target.value)} placeholder="Mínimo: 1 maiúscula + 1 número" className={inputClass} /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-2">Confirmar Senha *</label><input type="password" value={form.confirmarSenha} onChange={e => update('confirmarSenha', e.target.value)} placeholder="Repita a senha" className={inputClass} /></div>
            </div>
          )}
